...
$(function () {
    ...
    $("#dg_bspw_wsh").datagrid({
        ...
        collapsible: true,
        method: 'post',
        url: '/xiangmuguanli/pw_bushen_xm_wsh',
    ...
    var wsh = $('#dg_bspw_wsh').datagrid('getPager');
    ...
    $('#suoshuxueyuan').combobox({
        ...
        onChange: function (newValue, oldValue) {
            ...
            $('#dg_bspw_wsh').datagrid('load', { "xiangmu": dgtitle, "loingid": loingid, "searchquery": newValue });
        }
    });
});
...
function pwchakan_wsh(index) {
    $('#dg_bspw_wsh').datagrid('selectRow', index);
    var row = $('#dg_bspw_wsh').datagrid('getSelected');
    if (row) {
        var name = row.XmName;
        var id = row.ID;
...
function pw_caozuo_wsh(index) {
    ...
    //此处要参考一下编辑方法
    if (row) {
        $('#shenhe_dlg').dialog('open').dialog('setTitle', '评委审核');
        //alert(row);
        var name = row.XmName;
        var id = row.ID;
        var xmmulu = row.Xiangmumulu;
...
function pwchehui(value, row, index) {
    ...
    if (pingweishenhe == "未审核" && bmfzrtj == "已提交") {
...
function pl_shenhe() {
    ...
    var checkedItems = $('#dg_bspw_wsh').datagrid('getChecked');
