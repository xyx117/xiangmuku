$(function () {
    ...
    $("#dg_bsywzy_tg").datagrid({
        singleSelect: true,

        async: false,
        collapsible: true,
        method: 'post',
        url: '/xiangmuguanli/ywzy_bushen_xm_tg',
    ...
    var p = $('#dg_bsywzy_tg').datagrid('getPager');
    ...
    $('#suoshuxueyuan').combobox({
        ...
        onChange: function (newValue, oldValue) {
            ...
            $('#dg_bsywzy_tg').datagrid('load', { "xiangmu": dgtitle, "searchquery": newValue });
        }
    });
...
function ywzychakan_tg(index) {
    $('#dg_bsywzy_tg').datagrid('selectRow', index);
    var row = $('#dg_bsywzy_tg').datagrid('getSelected');
    if (row) {
        var name = row.XmName;
        var id = row.ID;
...
//    var checkedItems = $('#dg_bsywzy_tg').datagrid('getChecked');
...
//业务专员之评委评审统计图表
function tb_ywzy() {
    //$('#tb_ywzy_dlg').dialog('open').dialog('setTitle', '申报项目图表');   //将图表由弹出对话框改为新增页面
    //alert("sdf");
...
function jdtb_ywzy() {
    ...
    $('#tubiao').attr("src", "/xiangmuguanli/ywzy_pwjd_piechart?mulu=" + dgtitle + "&fgldshenhefou=0");
};
