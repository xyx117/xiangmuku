...
$(function () {
    ...
    $("#dg_bscwzg_btg").datagrid({
        singleSelect: true,
        async: false,
        //async: true,
        collapsible: true,
        method: 'post',
        url: '/xiangmuguanli/ywzy_bushen_xm_btg',
    ...
    var p_btg = $('#dg_bscwzg_btg').datagrid('getPager');
    ...
    $('#suoshuxueyuan').combobox({
        ...
        onChange: function (newValue, oldValue) {
            ...
            $('#dg_bscwzg_btg').datagrid('load', { "xiangmu": dgtitle, "searchquery": newValue });
        }
    });
});
...
function TitleFormatter(value, row, index) {
    ...
    if (value1 == null) {
        ...
        var ss = '<a href="#" title="' + value + '" onclick="cwzgchakan_btg(' + index + ')" class="easyui-tooltip">' + value1 + '</a>';
        return ss
    }
};
...
//财务主管查看财务主管查看不通过项目
function cwzgchakan_btg(index) {
    $('#dg_bscwzg_btg').datagrid('selectRow', index);
    var row = $('#dg_bscwzg_btg').datagrid('getSelected');
    if (row) {
        var name = row.XmName;
        var id = row.ID;
...
//审核流程日志，财务主管查看不通过项目
function cwzg_xmrizhi_btg(value, row, index) {
    //alert("sdf")
