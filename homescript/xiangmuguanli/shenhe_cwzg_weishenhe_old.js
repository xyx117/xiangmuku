$(function () {
    ...
    var p_wsh = $('#dg_shcwzg_wsh').datagrid('getPager');
    $(p_wsh).pagination({
    ...
    $('#suoshuxueyuan').combobox({
        ...
        onChange: function (newValue, oldValue) {
            ...
            $('#dg_shcwzg_wsh').datagrid('load', { "xiangmu": dgtitle, "searchquery": newValue });
        }
    });
});
...
function cwzgchakan_wsh(index) {
    $('#dg_shcwzg_wsh').datagrid('selectRow', index);
    var row = $('#dg_shcwzg_wsh').datagrid('getSelected');
    if (row) {
        var name = row.XmName;
        var id = row.ID;
...
function cwzg_xmrizhi_wsh(value, row, index) {
    //alert("sdf")
    return "<a href='javascript:void(0)' style='width:80%' class='rizhi' onclick='rizhichakan_wsh(" + index + ")' target='mainFrame'>日志</a>";
