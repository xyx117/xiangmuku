...
$(function () {
    ...
    var p = $('#dg_bsywzy_btg').datagrid('getPager');
    ...
    $('#suoshuxueyuan').combobox({
        ...
        onChange: function (newValue, oldValue) {
            ...
            $('#dg_bsywzy_btg').datagrid('load', { "xiangmu": dgtitle, "searchquery": newValue });
        }
    });
...
function ywzychakan_btg(index) {
    $('#dg_bsywzy_btg').datagrid('selectRow', index);
    var row = $('#dg_bsywzy_btg').datagrid('getSelected');
    if (row) {
        var name = row.XmName;
        var id = row.ID;
...
function ygxmrizhi_btg(value, row, index) {
    //alert("sdf")
    return "<a href='javascript:void(0)' style='width:90%' class='rizhi' onclick='rizhichakan_btg(" + index + ")' target='mainFrame'>日志</a>";
