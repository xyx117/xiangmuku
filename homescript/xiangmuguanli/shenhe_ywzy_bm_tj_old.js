...
function xm_bm_tj_chehui(value, row, index) {
    ...
    if (bmfzrtj == "已提交" && fgldshenhe == "未审核") {           //部门负责人的完善信息，只有在未审核和未提交时候才能使用  chehui_jin
...
function piliang_excel() {
    ...
    if (checkedItems.length == 0) {
        ...
        if (checkedItems) {
            ...
            window.location.href = "excel_out_piliang?xmid="+names;
        }
    }
};
...
//                $('#dg_shywzy_tg').datagrid('reload');	// reload the user data
