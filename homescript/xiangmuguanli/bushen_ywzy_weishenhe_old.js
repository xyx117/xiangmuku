$(function () {
    ...
    $("#dg_bsywzy_wsh").datagrid({
        ...
        onLoadSuccess: function (data) {
            ...
            $('.tianbiao_jin').linkbutton({ text: '填表', plain: true, disabled: true });
    ...
    var p = $('#dg_bsywzy_wsh').datagrid('getPager');
    ...
    $('#suoshuxueyuan').combobox({
        ...
        onChange: function (newValue, oldValue) {
            ...
            $('#dg_bsywzy_wsh').datagrid('load', { "xiangmu": dgtitle, "searchquery": newValue });
        }
    });
});
...
function ywzychakan_wsh(index) {
    $('#dg_bsywzy_wsh').datagrid('selectRow', index);
    var row = $('#dg_bsywzy_wsh').datagrid('getSelected');
    if (row) {
        var name = row.XmName;
        var id = row.ID;
...
function ywzy_shenhe_wsh(value, row, index) {
    ...
    if (ywzyshenhe == "未审核" && bmfzrtijiao == "已提交" && pingweitijiao == "已提交") {
        return "<a href='javascript:void(0)' style='width:95%' class='zhongshen' onclick='ywzy_caozuo_wsh(" + index + ")' target='mainFrame'>终审</a>";
    }
    //if (ywzyshenhe == "未审核" && bmfzrtijiao == "已提交" && pwshenhe == "通过") {
    //    return "<a href='javascript:void(0)' onclick='ywzy_caozuo_wsh_yc(" + index + ")' target='mainFrame'>撤回</a>";
    //}
};

////评委已经审核通过，不能再撤回
//function ywzy_caozuo_wsh_yc() {
//    $.messager.alert("错误提示", "评委已经审核通过，不能再撤回！", "warning");
//}

//审核操作弹出对话框
function ywzy_caozuo_wsh(index) {
    $('#fm_bsywzy').form('clear');
    $('#dg_bsywzy_wsh').datagrid('selectRow', index);
    var row = $('#dg_bsywzy_wsh').datagrid('getSelected');
    //此处要参考一下编辑方法
    if (row) {
        $('#shenhe_dlg').dialog('open').dialog('setTitle', '业务部门审核');
        //alert(row);
        var name = row.XmName;
        var id = row.ID;
        var xmmulu = row.Xiangmumulu;
    ...
    $('#fm_bsywzy').form('submit', {
        ...
        success: function (result) {
            ...
            if (result.success == true) {
                ...
                $('#dg_bsywzy_wsh').datagrid('reload');	// reload the user data
                $.messager.show({
                    title: '提示',
                    msg: result.errorMsg
                });
...
function pl_shenhe() {
    ...
    var checkedItems = $('#dg_bsywzy_wsh').datagrid('getChecked');
